...
- [docs/design-system.md](docs/design-system.md) — catalogo dei token e regolamento UI/UX. In caso di dubbio visivo, vince questo file. Le due superfici hanno un documento ciascuna, che descrivono **usi diversi degli stessi token**, mai un secondo sistema: [docs/design-system-admin.md](docs/design-system-admin.md) (area riservata, densa) e [docs/design-system-public.md](docs/design-system-public.md) (vetrina).
...
- **Due aree, un'applicazione**: la radice è il sito pubblico (`modules/site`), tutto ciò che richiede una sessione vive sotto `/admin`. La divisione la fa `meta.requiresAuth` in `router/adminArea.ts`, e `registerAdminArea` **pretende la prova restituita da `installAuthModule`**: senza il modulo Auth la guardia non esiste, quindi l'area privata non viene nemmeno registrata. Una pagina pubblica nuova non dichiara `requiresAuth`, dichiara `publicSite: true` e va aggiunta a `apps/web/prerender.config.mjs` (vedi [wiki/patterns/add-public-page.md](wiki/patterns/add-public-page.md)).
...
- **Libreria di riferimento:** `motion-v` (Motion per Vue). Questo repository standardizza su `motion-v` come la libreria JS per tutte le animazioni component-level e per le animazioni che coordinano più proprietà o elementi.
- **Regola obbligatoria:** ogni **componente** o **elemento UI** che implementa un'animazione (entrata/uscita, transizione complessa, layout motion, gesture-driven, hover/focus/active effects o choreografie multiple) **deve** usare `motion-v` e i suoi composables/componenti. Le animazioni puramente decorative e triviale (colore/opacità semplice) possono rimanere in CSS quando non richiedono orchestrazione JS, ma vanno comunque documentate e tokenizzate.
- **Hover / Focus / Active:** gli effetti di stato interattivo (hover, focus, active, pressed) devono usare `motion-v` quando involve una transizione animata; per cambiamenti di stato semplici basati su token (colore, opacità, ombra) si usano utility semantiche e le variabili CSS semantiche. In ogni caso, l'effetto dev'essere coerente con il design token e rispettare la distinzione tra movimento decorativo e informativo.
- **Accessibilità:** tutte le animazioni devono rispettare `prefers-reduced-motion` e prevedere una versione ridotta o assente per utenti che lo richiedono. I componenti devono esporre un comportamento deterministico a motion disabilitato.
- **Best practices:** evitare animazioni su `width`/`height`/`top`/`left`; preferire `transform` e `opacity`. Coordinare le durate con i token di movimento (`--duration-fast`, `--duration-base`). Documentare ogni animazione nel blocco commento del componente (scopo, fallback reduced-motion, token usati).
- **Verifica e test:** ogni story Storybook che mostra un'animazione deve avere una variante `prefers-reduced-motion` + test che assicuri il fallback. Le review del codice verificano che le animazioni component-level usino `motion-v` e non soluzioni puntuali non tokenizzate.

## Comandi (man mano che il repo cresce)

Dalla root: `pnpm install` e `dotnet build` devono sempre funzionare. `docker compose -f docker/docker-compose.yml up` avvia le dipendenze locali (PostgreSQL, Redis, Seq — Seq UI su http://localhost:5341). `dotnet run --project apps/api/src/Api` avvia l'API su http://localhost:5080 (health: `/health/ready`, OpenAPI: `/openapi/v1.json`). `pnpm --filter @enterprise/web dev` avvia il frontend su http://localhost:5173 con proxy `/api` verso l'API (credenziali di sviluppo: `admin@example.com` / `Password123!`, il bootstrap admin seedato solo in Development). Test: `dotnet test` (backend; gli integration usano Testcontainers, serve Docker attivo), `pnpm test` (frontend unit), `pnpm --filter @enterprise/web test:e2e` (Playwright, richiede l'API attiva in Development, avviata con `Modules__Auth__CredentialsPermitLimit=1000`: la suite intera esaurisce il budget di default e le sessioni smettono di rinnovarsi — vedi `apps/web/playwright.config.ts`). Migrations di un modulo: `dotnet ef migrations add <Nome> --project modules/<nome>/backend --startup-project apps/api/src/Api` (tool in `.config/dotnet-tools.json`, `dotnet tool restore`). Nuovo progetto dal template: `node scripts/create-project.mjs --name "Acme CRM" --target ../acme-crm` (vedi [docs/create-project.md](docs/create-project.md)). Build statica del sito pubblico (prerender + sitemap): `VITE_PUBLIC_BASE_URL=https://… pnpm --filter @enterprise/web build:static`. Controllo dei link della documentazione: `pnpm check:links` (in CI, fallisce su un link rotto). Se aggiungi script, documentali qui.
