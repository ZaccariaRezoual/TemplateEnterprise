...
- **Event-Driven**: i moduli comunicano pubblicando eventi di dominio sull'Event Bus, senza conoscere i destinatari. Il modulo Realtime (SignalR) distribuisce gli eventi ai client; le feature non usano mai SignalR direttamente (componente → composable → Realtime Service → SignalR Client).
- **Hub SignalR sottili**: solo gestione connessioni e routing; la logica sta nei Service. Gruppi gestiti esclusivamente server-side, connessioni autenticate via JWT.
